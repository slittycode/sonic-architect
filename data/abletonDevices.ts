...
const BAND_DEVICES: Record<string, BandDeviceMap> = {
  'Sub Bass': {
    dominant: {
      ...
      settings: 'Osc A: Sine, Coarse 0, Level 100%. Filter: LP 24dB, Freq 80Hz. Amp Env: A 0ms, D 0ms, S 100%, R 50ms.',
    },
    present: {
      device: 'Wavetable',
      preset: 'Sub Warmth',
      settings: 'Osc 1: Basic Shapes > Sine/Triangle blend. Filter 1: LP 12dB @ 120Hz. Sub Osc: On, -12st.',
    },
  },
  'Low Bass': {
    dominant: {
      device: 'Wavetable',
      preset: 'Analog Bass',
      settings: 'Osc 1: Basic Shapes > Saw. Osc 2: Square -12st. Filter 1: LP 18dB @ 250Hz, Res 20%. Drive: 15%.',
    },
    present: {
      device: 'Operator',
      preset: 'FM Bass',
      settings: 'Algorithm 1. Osc A: Sine. Osc B: Sine, Coarse 1, Fine 0, Level 40%. Filter: LP 12dB @ 300Hz.',
    },
  },
  'Low Mids': {
    dominant: {
      device: 'Analog',
      preset: 'Warm Pad',
      settings: 'Osc 1: Saw. Osc 2: Square, Fine +5. Filter: LP 12dB @ 500Hz, Res 15%. Chorus: On.',
    },
    present: {
      device: 'Wavetable',
      preset: 'Body Synth',
      settings: 'Osc 1: Spectral > Formant. Filter 1: BP 12dB @ 400Hz, Res 25%.',
    },
  },
  'Mids': {
    dominant: {
      device: 'Wavetable',
      preset: 'Lead Synth',
      settings: 'Osc 1: Basic Shapes > Saw. Unison: 4 voices, Amount 30%. Filter 1: LP 24dB @ 3kHz. Amp Env: A 5ms, D 200ms, S 70%, R 100ms.',
    },
    present: {
      device: 'Operator',
      preset: 'Electric Keys',
      settings: 'Algorithm 2. Osc A: Sine. Osc B: Sine, Coarse 2, Level 30%. Osc C: Sine, Coarse 3, Level 15%. Amp Env: A 2ms, D 800ms, S 40%, R 200ms.',
    },
  },
  'Upper Mids': {
    dominant: {
      device: 'Wavetable',
      preset: 'Pluck Lead',
      settings: 'Osc 1: Basic Shapes > Saw. Filter 1: BP 12dB @ 3.5kHz, Res 30%. Amp Env: A 1ms, D 300ms, S 20%, R 150ms.',
    },
    present: {
      device: 'Operator',
      preset: 'Bright Stab',
      settings: 'Algorithm 4. Osc A: Saw. Osc B: Square, Coarse 1, Level 50%. Filter: HP 6dB @ 1kHz + LP 12dB @ 6kHz.',
    },
  },
  'Highs': {
    dominant: {
      ...
      settings: 'Load a noise sample. Filter: HP 18dB @ 6kHz. Amp Env: A 0ms, D 80ms, S 0%, R 50ms. Spread: 80%.',
    },
    present: {
      device: 'Operator',
      preset: 'Noise Texture',
      settings: 'Osc D: Noise, Level 60%. Filter: BP 12dB @ 8kHz, Res 10%. Amp Env: A 0ms, D 150ms, S 15%, R 100ms.',
    },
  },
  'Brilliance': {
    dominant: {
      device: 'Operator',
      preset: 'Shimmer Air',
      settings: 'Osc A: Sine, Coarse 0. Osc B: Noise, Level 20%. Filter: HP 12dB @ 10kHz. Reverb Send: 60%.',
    },
    present: {
      device: 'Wavetable',
      preset: 'Airy Texture',
      settings: 'Osc 1: Spectral > Breathy. Filter 1: HP 12dB @ 8kHz. Chorus: On, Rate 0.3Hz.',
    },
  },
};
...
const FX_RULES: FXRule[] = [
  {
    ...
    recommendation: 'Ableton Glue Compressor: Threshold -15dB, Ratio 4:1, Attack 0.1ms, Release Auto. Makeup +3dB. Or Drum Buss for parallel compression.',
  },
  {
    condition: (f) => f.crestFactor >= 6 && f.crestFactor < 12,
    artifact: 'Moderate compression / controlled dynamics',
    recommendation: 'Ableton Compressor: Threshold -12dB, Ratio 3:1, Attack 10ms, Release 100ms. Use on bus groups for glue.',
  },
  {
    condition: (f) => f.crestFactor >= 12,
    artifact: 'High dynamic range — minimal compression',
    recommendation: 'Consider light bus compression: Glue Compressor, Ratio 2:1, Attack 30ms, Release Auto. Subtle gain reduction (1-3dB) for cohesion.',
  },
  {
    condition: (f) => {
    ...
    recommendation: 'Ableton Saturator on bass bus: Drive 5dB, Soft Clip mode. Adds harmonics that translate on small speakers. Follow with EQ Eight: HP at 30Hz to control rumble.',
  },
  {
    condition: (f) => f.spectralCentroidMean > 3000,
    artifact: 'Bright overall spectrum — high spectral centroid',
    recommendation: 'If harshness: EQ Eight, cut 2-4kHz by 2-3dB with wide Q. Ableton Channel EQ on master: reduce Highs by 1-2dB for smoother top end.',
  },
  {
    condition: (f) => f.spectralCentroidMean < 1000,
    artifact: 'Dark overall spectrum — low spectral centroid',
    recommendation: 'Add presence: EQ Eight, boost 3-5kHz by 2-3dB with a gentle Q. Ableton Exciter on master: Freq 3kHz, Drive 10-15%.',
  },
  {
    condition: (f) => f.onsetDensity > 8,
    artifact: 'High rhythmic density detected (many transients)',
    recommendation: 'Use Ableton Drum Buss for punch: Crunch 20%, Damp 8kHz, Comp Medium. Gate or sidechain to prevent mud from overlapping transients.',
  },
  {
    condition: (f) => f.onsetDensity < 2 && f.duration > 5,
    artifact: 'Low transient density — sustained/ambient content',
    recommendation: 'Ableton Reverb: Decay 3-5s, Size 80, Diffusion Network On. Or Hybrid Reverb with convolution IR for natural space. Chorus for width.',
  },
];
...
export function getInstrumentRecommendations(
...
): { element: string; timbre: string; frequency: string; abletonDevice: string }[] {
  const instruments: { element: string; timbre: string; frequency: string; abletonDevice: string }[] = [];
  ...
  for (const band of bands) {
    ...
    const timbreDesc = band.dominance === 'dominant'
 ...
 */
export function getSecretSauce(
  features: AudioFeatures
): { trick: string; execution: string } {
  // Find the most notable characteristic
  const dominantBands = features.spectralBands.filter(b => b.dominance === 'dominant');
  ...
  if (isBright && features.spectralBands.find(b => b.name === 'Brilliance')?.dominance !== 'absent') {
    return {
