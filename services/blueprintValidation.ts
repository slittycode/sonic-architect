...
export function validateBlueprint(data: unknown): ReconstructionBlueprint {
  ...
  const arrangement = requireArray(root.arrangement, 'arrangement').map(
    (entry, index) => {
      const row = requireRecord(entry, `arrangement[${index}]`);
      return {
        timeRange: requireString(
          row.timeRange,
          `arrangement[${index}].timeRange`
        ),
        label: requireString(row.label, `arrangement[${index}].label`),
        description: requireString(
          row.description,
          `arrangement[${index}].description`
        ),
      };
    }
  );

  const instrumentation = requireArray(
    ...
    return {
      ...
      frequency: requireString(
        row.frequency,
        `instrumentation[${index}].frequency`
      ),
  ...
  const secretSauce = {
    ...
    execution: requireString(
      secretSauceInput.execution,
      'secretSauce.execution'
    ),
  };
  ...
  if (root.meta != null) {
    ...
    meta = {
      ...
      analysisTime:
        optionalNumber(metaInput.analysisTime, 'meta.analysisTime') ?? 0,
